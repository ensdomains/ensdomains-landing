...
The **uni.eth** integration served four main purposes:

1.  **Improved User Experience:** Replaced long, error-prone wallet addresses with memorable names.
2.  **Strengthened Brand Identity:** Reinforced the Uniswap brand with every ***.uni.eth** username.
3.  **Future-Proof Namespace:** Established a global, expandable, and upgradable namespace on **uni.eth**.
4.  **Interoperability:** Leveraged ENS's native multichain and cross-platform capabilities.
...
The system consists of three main components working together to deliver a seamless user experience:

- **ENS Parent Domain (**uni.eth**):** Managed by Uniswap Labs, this domain serves as the root for all Uniswap usernames.
- **Off-chain Resolver:** A custom backend service that handles subname issuance and resolution. It connects names to addresses without requiring on-chain transactions, making the process gas-free for users.
- **Frontend Integration:** A user flow built directly into the Uniswap mobile app and browser extension, allowing users to claim and manage their **uni.eth** usernames.
...
-   **July 6, 2024:** Uniswap grants access to a waitlisted browser extension for 100,000 early adopters.
-   **July 9, 2024:** An additional 250,000 usernames are whitelisted for the extension.
-   **November 2024:** The 1 million registered username milestone is reached.
...
![](./5.webp "Uniswap mobile app")

![](./6.webp "Uniswap Web Extension: Account creation flow, Step 1 and Step 2")
...
At the same time, displaying the bare handle creates a cleaner, chain-agnostic UI. This aligns with the multi-chain functionalities of the ENS protocol, where a single name can point to addresses on various blockchains.
...
1.  **Claim a branded `.eth` domain:** Secure a name that reflects your brand (e.g., `yourbrand.eth`).<br/>This becomes the root for all user subdomains.
2.  **Deploy a subname registry:** Implement a service (typically off-chain for speed and cost-effectiveness) to manage subname creation and resolution.
3.  **Deliver a seamless UI:** Build a user-friendly interface within your application for name issuance and profile management, abstracting away the underlying complexity.
