...
export const SquareAnimation = ({ bgColor, color, noBackground = false, ...props }: SquareAnimationProps) => {
  ...
  return (
    <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <defs>
        ...
        {!noBackground && <BGPattern fill={`var(--${bgColor})`} id={`bg-pattern-${bgColor}`} />}
      </defs>
      <g clipPath="url(#clip-overflow-200x200)">
        {!noBackground && (
          <>
            <rect width="200" height="200" fill="#F6F6F6" />
            <rect width="200" height="200" fill={`url(#bg-pattern-${bgColor})`} />
          </>
        )}
...
export const RectangleAnimation = ({ bgColor, color, width = 400, noBackground = false, ...props }: RectangleAnimationProps) => {
  const timing = generateMotionTiming({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${noBackground ? 58 : 100}`} fill="none" xmlns="http://www.w3.org/2000/svg" width={width} {...props}>
      <defs>
      ...
      <g clipPath={`url(#clip-overflow-${width}x100)`}>
        {!noBackground && (
          <>
            ...
            <rect width={width} height="100" fill={`url(#bg-pattern-${bgColor})`} />
          </>
        )}
        <path d={`M0 ${noBackground ? 29 : 46}H${width}`} stroke={`var(--${color})`} strokeWidth="58" />
        <path d={`M0 ${noBackground ? 29 : 46}H${width}`} stroke="#F6F6F6" strokeWidth="2" />
        <rect x={width} y={noBackground ? cubeSize / 2 - 4 : cubeSize - 4} width={cubeSize} height={cubeSize} rx="2" fill="#F6F6F6">
          <animateMotion
            begin={`${Math.random() * 3}s`}
            dur={`${timing.duration}s`}
            repeatCount="indefinite"
...
export const CornerAnimation = ({ bgColor, color, noBackground = false, ...props }: CornerAnimationProps) => {
  const timing = generateMotionTiming({
    minDuration: 14,
    maxDuration: 18,
    pausePoints: {
